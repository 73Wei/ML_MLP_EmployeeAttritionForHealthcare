...
df = pd.read_excel("資料清洗後標準化.xlsx",0) #('https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_time_series/time_series_covid19_confirmed_global.csv')
print(df.columns)

#colX=['Age', 'Sex' , 'ALB', 'ALP',   'ALT', 'AST', 'BIL', 'CHE', 'CHOL', 'CREA', 'GGT', 'PROT']
# sepal length (cm)	sepal width (cm)	petal length (cm)	petal width (cm)	target
# 為了畫出 決策區域， 只能用2 個
colX=['DailyRate','PercentSalaryHike']  # <------ 為了畫出 決策區域， 只能用2 個
colY=['PerformanceRating']
...
print("KMeans 演算法 預估答案：", y_predict,  "KMeans 演算法 準確率：", kmeansScore,
      "\nKMeans 演算法 預估答案：", y_predict, " KMeans 演算法 修正後準確率：", kmeanshomogeneity_score)
